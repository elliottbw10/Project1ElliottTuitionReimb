...
function getForm(){
	...
	xhr.open("GET", "http://localhost:8080/TuitionReimb/bForm?formid="+formid,true);
	xhr.send();
}
...
function postForm(){
    ...
    xhr.open("POST","http://localhost:8080/TuitionReimb/bForm",true);
...
function getAmt(){
	...
	xhr.open("GET", "http://localhost:8080/TuitionReimb/benAd?Userid="+Userid,true);
	xhr.send();
}
